...
fun main() {
    ...
    val pattern = Regex("public static Item ([A-Z0-9_]+) = (.*);")
    ...
    pattern.findAll(contents).forEach {
        ...
        initContents += "        $varName = registry.registerItem(INSTANCE.compatId(\"$varName\"), () -> $instanceGenerator);\n"
    }

    contents = contents.replace(pattern) {
        val (varName, instanceGenerator) = it.destructured
        "public static RegistryResult<Item> $varName;"
    }
